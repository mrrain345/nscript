...
impl<'a, T> Default for AppendList<'a, T> {
    fn default() -> Self {
        Self::new()
    }
}

impl<'a, T> Index<usize> for AppendList<'a, T> {
    type Output = T;
...
impl<'a, T> FromIterator<T> for AppendList<'a, T> {
...
impl<'a, T> IntoIterator for &'a AppendList<'a, T> {
    type Item = &'a T;
...
impl<'a, T: PartialEq> PartialEq for AppendList<'a, T> {
...
impl<'a, T: Debug> Debug for AppendList<'a, T> {
