...
pub fn less_or_equal<'ctx>(env: &mut Environment<'ctx>, left: &Expression, right: &Expression) -> AnyValue<'ctx> {
  ...
  match (left, right) {
    ...
    (AnyValue::Integer(left), AnyValue::Integer(right)) => {
      let value = env.builder.build_int_compare(IntPredicate::SGT, left, right, "less_or_equal");
      AnyValue::Boolean(value)
    },
    // Number <= Number
    (AnyValue::Number(left), AnyValue::Number(right)) => {
      let value = env.builder.build_float_compare(FloatPredicate::OGT, left, right, "less_or_equal");
      AnyValue::Boolean(value)
    },
    // Integer <= Number
    ...
    _ => panic!("Parser error: Less or equal of incompatible types")
  }
}