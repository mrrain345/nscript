...
pub fn add<'ctx>(env: &mut Environment<'ctx>, left: &Expression, right: &Expression) -> AnyValue<'ctx> {
  ...
  match (left, right) {
    ...
    (AnyValue::Integer(left), AnyValue::Integer(right)) => {
      let value = env.builder.build_int_add(left, right, "add");
      AnyValue::Integer(value)
    },
    // Number + Number
    (AnyValue::Number(left), AnyValue::Number(right)) => {
      let value = env.builder.build_float_add(left, right, "add");
    ...
    _ => panic!("Parser error: Add of incompatible types")
  }
}